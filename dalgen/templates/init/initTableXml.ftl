...
<table sqlname="${table.sqlName}" physicalName="${table.physicalName}"<#if table.remark??> remark="${table.remark!}"</#if>>
    ...
    <#if table.primaryKeys??>
        ...
        <operation name="updateBySelective" paramtype="object" remark="根据主键ID动态更新表:${table.sqlName}">
            UPDATE ${table.sqlName}
            <trim prefix="SET" suffixOverrides=",">
                    ...
                    <#list table.columnList as column>
                        ...
                        <if test="${column.javaName} != null">${column.sqlName}${lib.space(column.sqlName)} = ${lib.updateVal(column)} ,</if>
                        </#if>
                    </#list>
            </trim>
            WHERE
        ...
        <operation name="getBySelective" paramtype="object" multiplicity="many" remark="根据传入条件获取数据:${table.sqlName}">
            SELECT * FROM ${table.sqlName}
        ...
        <operation name="insertBySelective" paramtype="object" remark="动态插入表:${table.sqlName}">
            <#if dalgen.dbType=="MySQL">
                <selectKey resultType="java.lang.Long" keyProperty="id" order="AFTER">
                    SELECT
                    LAST_INSERT_ID()
